...
def initialize_mdp_data(num_states):
    ...
    return {
        ...
        'reward': reward,
        'value': value, # this is your value "function"
...
def choose_action(state, mdp_data):
    ...
    # find the argmax a of the Bellman Equation

    # find transitional probs for state
    transition_probs_s = mdp_data['transition_probs'][state,:,:]

    # calcualte prbability weighted value paths
    expected_values = V.dot(transition_probs_s)

    # decide which action to take
    if expected_values[0] == expected_values[1]:
        a = np.random.choice([0,1])
    else:
        a = np.argmax(expected_values)
    return a
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    trans_probs = mdp_data['transition_probs']
    reward_counts = mdp_data['reward_counts'] 
    
    dims = trans_counts.shape
    ...
    for base_state in range(num_states): # go through the state jump matrices by start state
        base_state_counts = trans_counts[base_state]
        for action in range(num_actions): # go through each action separately
            jump_counts = base_state_counts[:,action]
            if np.sum(jump_counts) != 0: # state-pair never tried
                affected = base_state
                trans_probs[base_state,:,action] = jump_counts / np.sum(jump_counts)
                #print(jump_counts / np.sum(jump_counts))
    
        # update rewards
        if reward_counts[base_state,1] > 0:
            mdp_data['reward'][base_state] = reward_counts[base_state,0] / reward_counts[base_state,1]
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        new_V = mdp_data["reward"] + gamma * np.max(mdp_data["value"].dot(mdp_data['transition_probs']), axis=1)
        if np.max(np.abs(new_V - mdp_data["value"])) < tolerance:
...
def main(plot=True):
    ...
    pause_time = 0.0001
    min_trial_length_to_start_display = 5
