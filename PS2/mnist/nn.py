...
def softmax(x):
    ...
    softmax_mat = exp_mat / np.sum(exp_mat, axis=1, keepdims=True)
...
def forward_prop(data, labels, params):
    ...
    # average loss
    avg_loss = -np.sum(labels * np.log(sm_output), axis=1).mean()

    return a, sm_output, avg_loss
    # *** END CODE HERE ***
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    # fetch forward pass data
    a, sm, _ = forward_prop_func(data, labels, params)

    # len data for averages
    n = len(data)

    # compute gradients Input size x num-next layer
    ## first for neurons
    dJ_dz2 = sm - labels
    dJ_da1 = dJ_dz2 @ W2.T
    dJ_dz1 = a * (1-a) * dJ_da1
    
    ## then for weights and biases
    W2_g = (1/n) * (a.T @ dJ_dz2) + 2 * reg * W2
    b2_g = (dJ_dz2).mean(axis=0)
    W1_g = (1/n) * (data.T @ dJ_dz1) + 2 * reg * W1
    b1_g = (dJ_dz1).mean(axis=0)

    gradient_dict = {"W1": W1_g, "W2": W2_g, "b1": b1_g, "b2": b2_g}
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for start_idx in range(n-1)[::batch_size]:
        end_idx = min(start_idx + batch_size, n)

        batch_train = train_data[start_idx:end_idx]
        batch_test = train_labels[start_idx:end_idx]

        gradient_dict = backward_prop_func(batch_train, batch_test, params, forward_prop_func)

        # update params
        for param in params.keys():
             params[param] -= (learning_rate * gradient_dict[param])
    # *** END CODE HERE ***
