...
def initial_state():
    ...
    # *** START CODE HERE ***
    # State: 
    # B - helper constants to for kernal trick
    # X - design matrix
    # G - Gram matrix of kernel using X 
    return {"B":np.array([]), "X": np.array([])}
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    # find the hypothesis value h
    if len(B) == 0:
        h = 1
        state["X"] = np.array(x_i).reshape(1,-1)
    else:
        combo = B.dot(kernel(X, x_i))
        h = sign(combo)
        state["X"] = np.vstack([state["X"], x_i.reshape(1,-1)])
    
    b_i = learning_rate * (y_i - h)
...
def non_psd_kernel(a, b):
    ...
    #     return -1
    # return 0
    dif = np.sqrt(np.sum((a - b) ** 2, axis = 1))
    result = np.isclose(dif, 0).astype(int) * -1
    return result
